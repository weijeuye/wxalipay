...
public class UploadFileUtil {
    ...
    public static String flowUpload(MultipartFile[] attach, String portraitPath, String path) throws MyException {
        ...
        if (attach != null && attach.length>0){
            ...
            UploadFileUtil.portraitPath = UploadFileUtil.portraitPath + PATH_IMG + path + "/" + date + "/";
            String uploadPath="";
            checkDirectories(UploadFileUtil.portraitPath);
                ...
                if (!attach[i].isEmpty()) {
                    ...
                    if (attach[i].getSize() > filesize) {
                        ...
                        try {
                            ...
                            out = new FileOutputStream(UploadFileUtil.portraitPath + fileName);
                            out.write(attach[i].getBytes());
                            out.flush();
                            out.close();
                            if (i==0){
                                uploadPath+= PATH_IMG + path + "/" + date + "/"+fileName;
                                log.info(UploadFileUtil.portraitPath +"图片路径");
                            }else {
                                uploadPath+=","+ PATH_IMG + path + "/" + date + "/"+fileName;
                            }
                        } catch (Exception e) {
